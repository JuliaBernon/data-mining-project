...
def parseMean(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        mean = data[3].split(":")[-1].split("\n")[0]
    return mean

def parseMeanBis(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        mean_bis = data[4].split(":")[-1].split("\n")[0]
    return mean_bis

def parseStd(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        std = data[6].split(":")[-1].split("\n")[0]
    return std

def parseStdBis(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        std_bis = data[7].split(":")[-1].split("\n")[0]
    return std_bis

def parseMin(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        min_dist = data[9].split(":")[-1].split("\n")[0]
    return min_dist

def parseMinBis(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        min_dist_bis = data[10].split(":")[-1].split("\n")[0]
    return min_dist_bis

def parseMax(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        max_dist = data[12].split(":")[-1].split("\n")[0]
    return max_dist

def parseMaxBis(textFile):
    with open(textFile, "r") as file:
        data = file.readlines()
        max_dist_bis = data[13].split(":")[-1].split("\n")[0]
    return max_dist_bis
